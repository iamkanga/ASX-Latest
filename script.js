// File Version: v123
// Last Updated: 2025-06-30 (Fixes for theme scope, PWA, sort persistence)
...
function applyTheme(themeName) {
    ...
    if (themeName && themeName !== 'none') {
        ...
        applyDefaultLightDarkTheme(); // Call the now globally accessible function
    }
    updateThemeToggleAndSelector();
}

// NEW: This function is now in the global scope
...
async function initializeAppLogic() {
    ...
    // updateMainButtonsState(false); // Removed this, as it's handled by onAuthStateChanged
    if (loadingIndicator) loadingIndicator.style.display = 'block';
    ...
    if (sortSelect) {
        sortSelect.addEventListener('change', async () => { // Made async to await save
            sortShares();
            await saveSortOrderPreference(sortSelect.value); // Save the selected sort order
        });
    }
    ...
    if (window.firestoreDb && window.firebaseAuth && window.getFirebaseAppId && window.firestore && window.authFunctions) {
        ...
        window.authFunctions.onAuthStateChanged(auth, async (user) => {
            if (user) {
                ...
                updateMainButtonsState(true);
                // Removed loadingIndicator.style.display = 'none'; here, as loadUserWatchlists will handle it
