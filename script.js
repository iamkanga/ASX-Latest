// File Version: v152
// Last Updated: 2025-07-02 (Share Research Feature Logic)
...
async function subscribeToWatchlists() {
    ...
    if (!userPath || !window.firestore || !window.firestore.collection) {
        console.warn("Firestore or user path not ready for watchlists subscription.");
        return;
    }

    const watchlistsColRef = window.firestore.collection(userPath, 'watchlists');
    unsubscribeWatchlists = window.firestore.onSnapshot(watchlistsColRef, (snapshot) => {
        ...
        if (!defaultWatchlistExists) {
            ...
            const defaultWatchlistRef = window.firestore.doc(watchlistsColRef, DEFAULT_WATCHLIST_ID_SUFFIX);
            window.firestore.setDoc(defaultWatchlistRef, { name: DEFAULT_WATCHLIST_NAME, createdAt: window.firestore.FieldValue.serverTimestamp() }, { merge: true })
                .then(() => {
...
async function subscribeToShares() {
    ...
    let q;
    const sharesCollectionRef = window.firestore.collection(userPath, 'shares');
    ...
    if (currentSelectedWatchlistIds.includes("ALL_SHARES_ID")) {
        ...
        q = window.firestore.query(sharesCollectionRef, window.firestore.where('watchlists', 'array-contains-any', currentSelectedWatchlistIds));
...
async function saveShare() {
    ...
    try {
        ...
        if (selectedShareDocId) {
            ...
            // Add new share
            shareData.entryDate = window.firestore.FieldValue.serverTimestamp(); // Set entry date only for new shares
...
async function deleteShare(shareId) {
    ...
    try {
        ...
        const userPath = getUserDataPath();
        if (!userPath) return;
        const shareDocRef = window.firestore.doc(window.firestore.collection(userPath, 'shares'), shareId);
...
async function addWatchlist() {
    ...
    try {
        ...
        const userPath = getUserDataPath();
        if (!userPath) return;
        await window.firestore.addDoc(window.firestore.collection(userPath, 'watchlists'), {
            name: watchlistName,
            createdAt: window.firestore.FieldValue.serverTimestamp()
        });
...
async function saveWatchlistName() {
    ...
    try {
        ...
        const userPath = getUserDataPath();
        if (!userPath) return;
        const watchlistDocRef = window.firestore.doc(window.firestore.collection(userPath, 'watchlists'), selectedWatchlistId);
...
async function deleteWatchlist() {
    ...
    try {
        ...
        const watchlistDocRef = firestore.doc(firestore.collection(userPath, 'watchlists'), selectedWatchlistId);
        batch.delete(watchlistDocRef);

        // 2. Remove this watchlist from any shares that belong to it
        const sharesCollectionRef = firestore.collection(userPath, 'shares');
        const q = firestore.query(sharesCollectionRef, firestore.where('watchlists', 'array-contains', selectedWatchlistId));
        ...
        querySnapshot.forEach(shareDoc => {
            ...
            const shareRef = firestore.doc(sharesCollectionRef, shareDoc.id);
...
async function saveResearchedShareToWatchlist() {
    ...
    const shareData = {
        ...
        entryDate: window.firestore.FieldValue.serverTimestamp() // New entry
    };
    ...
    try {
        ...
        const existingShareQuery = window.firestore.query(
            sharesCollectionRef,
            window.firestore.where('shareName', '==', shareName),
            window.firestore.where('watchlists', 'array-contains', selectedWatchlistId)
        );
        const existingShareSnapshot = await window.firestore.getDocs(existingShareQuery);
        ...
        if (!existingShareSnapshot.empty) {
            ...
            if (confirmUpdate) {
                // Update existing share
                await window.firestore.updateDoc(existingShareDoc.ref, shareData);
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    if (window.firestoreDb && window.firebaseAuth && window.getFirebaseAppId && window.firestore && window.authFunctions) {
        ...
        if (!window._appLogicInitialized) {
            ...
            // The onAuthStateChanged listener will then trigger initializeAppLogic
