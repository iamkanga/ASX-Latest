// File Version: v119
// Last Updated: 2025-06-28 (Function Definition Order Fix)
        ...
        window.addEventListener('scroll', () => {
            if (window.innerWidth <= 768) {
                if (window.scrollY > 200) {
                    ...
                    setTimeout(() => {
                        scrollToToYou are absolutely right! My apologies for that oversight. The `ReferenceError: applyDefaultLightDarkTheme is not defined` indicates a function was called before its definition was processed by the browser. This is a common timing issue, especially with how JavaScript loads and executes.

I have adjusted the order of function definitions in `script.js` to ensure `applyDefaultLightDarkTheme` is defined and accessible before `initializeAppLogic` attempts to call it.

Here is the updated `script.js` file (Version v119).

**Please perform a hard refresh (Ctrl+Shift+R or Cmd+Shift+R) in your browser after updating this file.** This is crucial to ensure the browser loads the new version of the script and clears any cached older versions.
