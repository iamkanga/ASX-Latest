...
function updateMainButtonsState(enable) {
    ...
    // saveCashBalancesBtn is handled by checkCashBalancesDirtyState
...
async function loadCashCategories() {
    ...
    if (!db || !currentUserId || !window.firestore) {
        ...
        userCashCategories = [];
        cashAssetVisibility = {}; // Clear visibility state too
    ...
    try {
        ...
        unsubscribeCashCategories = window.firestore.onSnapshot(q, (querySnapshot) => {
            ...
            let fetchedVisibility = {}; // Temporary object to build visibility state

            querySnapshot.forEach((doc) => {
                const category = { id: doc.id, ...doc.data() };
                fetchedCategories.push(category);
                // Initialize cashAssetVisibility from Firestore's isHidden property
                if (category.hasOwnProperty('isHidden')) {
                    fetchedVisibility[category.id] = category.isHidden;
                } else {
                    fetchedVisibility[category.id] = false; // Default to visible if property is missing
                }
            });

            userCashCategories = fetchedCategories.sort((a, b) => a.name.localeCompare(b.name));
            cashAssetVisibility = fetchedVisibility; // Update the global visibility state
            logDebug('Cash Categories: Data updated from snapshot. Total categories: ' + userCashCategories.length);
            logDebug('Cash Categories: Visibility state updated from snapshot: ', cashAssetVisibility);
...
function renderCashCategories() {
    ...
    if (userCashCategories.length === 0) {
        ...
        logDebug('Cash Categories: No categories, showing empty message and disabling save button.');
    ...
    userCashCategories.forEach(category => {
        ...
        hideToggleButton.addEventListener('click', (event) => {
            ...
            toggleCashAssetVisibility(category.id); // This now updates Firestore
        });
...
async function saveCashCategories() {
    ...
    const currentDataForSave = userCashCategories.map(category => ({
        ...
        // Get the current visibility state for this asset. Default to false (visible) if not explicitly set.
        isHidden: cashAssetVisibility[category.id] === true // isHidden in DB means it's hidden, so true if cashAssetVisibility[id] is false
    }));
    ...
    if (areCashBalancesEqual(originalCashBalances, currentDataForSave)) {
    ...
    for (const category of currentDataForSave) {
        ...
        if (existingData) {
            // Update existing category if anything has changed (name, balance, or isHidden)
            const updatePayload = {};
            ...
            // Check for change in isHidden status
            const currentIsHiddenInDb = existingData.isHidden === true; // Treat undefined/false as false
            if (currentIsHiddenInDb !== category.isHidden) {
                updatePayload.isHidden = category.isHidden;
                needsUpdate = true;
            }
    ...
    try {
        if (categoriesProcessedCount > 0) {
            ...
            originalCashBalances = currentDataForSave.map(c => ({...c}));
...
function areCashBalancesEqual(data1, data2) {
    ...
    for (let i = 0; i < sortedData1.length; i++) {
        ...
        // Compare ID, name, balance, and isHidden
...
function checkCashBalancesDirtyState() {
    if (!saveCashBalancesBtn) {
        console.warn('checkCashBalancesDirtyState: saveCashBalancesBtn not found.');
        return;
    }
    ...
    // Compare with the `originalCashBalances` snapshot
...
/**
 * Toggles the visibility of a cash asset and persists the state to Firestore.
 * @param {string} assetId The ID of the cash asset to toggle.
 */
async function toggleCashAssetVisibility(assetId) {
    if (!db || !currentUserId || !window.firestore) {
        showCustomAlert('Firestore not available. Cannot toggle asset visibility.');
        return;
    }
    ...
    // Persist the new visibility state to Firestore
    try {
        const assetDocRef = window.firestore.doc(db, 'artifacts/' + currentAppId + '/users/' + currentUserId + '/cashCategories', assetId);
        await window.firestore.updateDoc(assetDocRef, { isHidden: newIsHidden });
        logDebug('Firestore: Cash asset (ID: ' + assetId + ') visibility updated to isHidden: ' + newIsHidden);
        // No need for showCustomAlert here, as the UI update is immediate and feedback is visual.
    } catch (error) {
        console.error('Firestore: Error updating cash asset visibility:', error);
        showCustomAlert('Error updating asset visibility: ' + error.message);
        // Revert UI state if Firestore update fails
        cashAssetVisibility[assetId] = currentIsHidden;
        if (assetElement) { // Revert UI
            if (currentIsHidden) {
                assetElement.classList.add('hidden');
                assetElement.querySelector('.hide-toggle-btn').innerHTML = '<i class="fas fa-eye-slash"></i>';
                assetElement.querySelector('.hide-toggle-btn').title = 'Show Asset';
    ...
    calculateTotalCash();
    checkCashBalancesDirtyState(); // Check dirty state after visibility change
}
