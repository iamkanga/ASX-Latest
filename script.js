// File Version: v151
// Last Updated: 2025-07-03 (Firebase Global Access Fix)
...
function applyTheme(themeName) {
    ...
    console.log("[Theme UI] Color theme select updated to:", themeName); // Log the theme update
}
...
function updateMainButtonsState(enable) {
    ...
    // Specific logs for select elements
    if (sortSelect) console.log("[UI State] Sort Select Disabled:", sortSelect.disabled);
    if (watchlistSelect) console.log("[UI State] Watchlist Select Disabled:", watchlistSelect.disabled);
}
...
function calculateDividendYieldsAndEstimatedDividend(
    ...
    if (!priceInput || !dividendInput || !frankingInput || !unfrankedYieldSpan || !frankingInput || !investmentValueSelect || !estimatedDividendSpan) {
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    const waitForFirebaseGlobalsAndInit = (attempts = 0) => {
        ...
        if (window.firebaseGlobalsReady && window.firestoreFunctions && window.authFunctions) {
            ...
            // Assign global Firebase instances to local variables *here*, after they are confirmed ready
