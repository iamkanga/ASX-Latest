...
self.addEventListener('install', (event) => {
    ...
    event.waitUntil(
        caches.open(CACHE_NAME)
            ...
            .catch((error) => {
                console.error('Service Worker v50: Cache addAll failed:', error); // Updated log
            })
    );
});

self.addEventListener('activate', (event) => {
    console.log('Service Worker v50: Activating...'); // Updated log for version
    event.waitUntil(
    ...
    if (event.request.method === 'GET') {
        event.respondWith(
            caches.match(event.request).then(cachedResponse => {
                ...
                if (cachedResponse) {
                    // console.log(`Service Worker v50: Serving from cache: ${event.request.url}`); // Updated log
                    return cachedResponse;
                }

                // Otherwise, fetch from the network
                // console.log(`Service Worker v50: Fetching from network: ${event.request.url}`); // Updated log
