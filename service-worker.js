...
const CACHE_NAME = 'asx-tracker-v30'; 
...
self.addEventListener('install', (event) => {
    console.log('Service Worker v30: Installing...'); // Updated log for version
    event.waitUntil(
        caches.open(CACHE_NAME)
            .then((cache) => {
                console.log('Service Worker v30: Cache opened'); // Updated log
                return cache.addAll(CACHED_ASSETS);
            })
            .then(() => {
                console.log('Service Worker v30: All assets added to cache. Calling skipWaiting.'); // Updated log
                return self.skipWaiting(); // Force the new service worker to activate immediately
            })
            .catch(error => {
                console.error('Service Worker v30: Installation failed:', error); // Updated log
            })
    );
});

self.addEventListener('activate', (event) => {
    console.log('Service Worker v30: Activating...'); // Updated log
    event.waitUntil(
        caches.keys().then((cacheNames) => {
            return Promise.all(
                cacheNames.map((cacheName) => {
                    if (cacheName !== CACHE_NAME) {
                        console.log(`Service Worker v30: Deleting old cache: ${cacheName}`); // Updated log
        ...
        event.respondWith(
            caches.match(event.request).then((cachedResponse) => {
                const fetchPromise = fetch(event.request).then((networkResponse) => {
                    ...
                    console.error(`Service Worker v30: Network fetch failed for ${event.request.url}.`, error); // Updated log
                ...
                console.error(`Service Worker v30: Cache match failed for ${event.request.url}.`, error); // Updated log
...
self.addEventListener('message', (event) => {
    if (event.data && event.data.type === 'SKIP_WAITING') {
        self.skipWaiting();
        console.log('Service Worker v30: Skip waiting message received, new SW activated.'); // Updated log
    }
});
