...
const CACHE_NAME = 'asx-tracker-v55'; 
...
self.addEventListener('install', (event) => {
    console.log('Service Worker v55: Installing...'); // Updated log for version
    event.waitUntil(
        caches.open(CACHE_NAME)
            .then((cache) => {
                console.log('Service Worker v55: Cache opened'); // Updated log
                return cache.addAll(CACHED_ASSETS);
            })
            .then(() => self.skipWaiting()) // Activate new service worker immediately
            .catch((error) => {
                console.error('Service Worker v55: Cache addAll failed:', error); // Updated log
            })
    );
});

self.addEventListener('activate', (event) => {
    console.log('Service Worker v55: Activating...'); // Updated log for version
    event.waitUntil(
        caches.keys().then((cacheNames) => {
            return Promise.all(
                cacheNames.map((cacheName) => {
                    if (cacheName !== CACHE_NAME) {
                        console.log('Service Worker v55: Deleting old cache:', cacheName); // Updated log
    ...
    if (event.request.method === 'GET') {
        event.respondWith(
            caches.match(event.request).then(cachedResponse => {
                ...
                if (cachedResponse) {
                    // console.log(`Service Worker v55: Serving from cache: ${event.request.url}`); // Updated log
                    return cachedResponse;
                }

                // Otherwise, fetch from the network
                // console.log(`Service Worker v55: Fetching from network: ${event.request.url}`); // Updated log
                const fetchPromise = fetch(event.request).then(networkResponse => {
                    ...
                    console.error(`Service Worker v55: Network fetch failed for ${event.request.url}.`, error); // Updated log
                ...
                console.error(`Service Worker v55: Cache match failed for ${event.request.url}.`, error); // Updated log
...
self.addEventListener('message', (event) => {
    if (event.data && event.data.type === 'SKIP_WAITING') {
        self.skipWaiting();
        console.log('Service Worker v55: Skip waiting message received, new SW activated.'); // Updated log
    }
});
